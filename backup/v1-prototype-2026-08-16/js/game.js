...
function ptext(str, gx, gy, size = 8, color = '#e8d9c0', align = 'left', alpha = 1) {
  ...
  let x = gx * U;
  if (align === 'center') x -= e.img.width * U / 2;
  else if (align === 'right') x -= e.img.width * U;
  uictx.imageSmoothingEnabled = false;
  uictx.globalAlpha = alpha;
  uictx.drawImage(e.img, Math.round(x), Math.round(gy * U), e.img.width * U, e.img.height * U);
  uictx.globalAlpha = 1;
  return e.w;
}
...
function invEntries() {
  ...
  const rows = [
    ...
    { id: 'tech', icon: Sprites.techIcon, label: `Tech component × ${player.inv.tech}` },
  ];
...
function drawProp(p, x, y) {
  ...
  if (T === 'scrap') { img = Sprites.scrapPiles[p.v]; oyOff = -20; drawShadow(x, y, 9); }
...
function drawPlayer(x, y) {
  ...
  ctx.drawImage(Sprites.player[player.frame], Math.round(x - 8), Math.round(y - 21));
  ctx.globalAlpha = 1;

  // melee swing: the weapon sweeps through an arc with a fading trail
  if (player.swing > 0 && player.melee) {
    const prog = 1 - player.swing / 0.22;                 // 0 → 1
    const start = player.angle - 1.3;
    const a = start + prog * 2.6;                         // current sweep angle
    // fading arc trail behind the weapon
    ...
    ctx.stroke();
    ctx.lineWidth = 1;
    // the weapon itself, rotating through the swing
    ctx.save();
    ctx.translate(x, y - 9);
    ctx.rotate(a);
    if (player.melee === 'pipe') {
...
function drawScrapper(x, y) {
  ...
  if (s.hp < s.maxHp) {
    ...
    ctx.fillRect(Math.round(x - 8), Math.round(y - 21), Math.round(16 * s.hp / s.maxHp), 2);
  }
}
...
function uiFrame(gx, gy, gw, gh) {
  uiRect(gx, gy, gw, gh, 'rgba(10,8,6,0.88)');
...
function drawHUD() {
  ...
  uiRect(8, VIEW_H - 10, 42, 3, '#3a1410');
  uiRect(8, VIEW_H - 10, Math.round(42 * player.hp / player.maxHp), 3, player.hp > 30 ? '#c94f2e' : '#ff3020');

  // weapon slots (bottom-right): equipped melee + gun
  uiRect(VIEW_W - 72, VIEW_H - 21, 66, 16, 'rgba(0,0,0,0.55)');
  if (player.melee) {
    const mi = player.melee === 'pipe' ? Sprites.pipeIcon : Sprites.knifeIcon;
    uiIcon(mi, VIEW_W - 69, VIEW_H - 16);
  }
  if (player.hasGun) {
    uiIcon(Sprites.pistolIcon, VIEW_W - 44, VIEW_H - 19);
    ptext('' + player.ammo, VIEW_W - 19, VIEW_H - 15, 8, player.ammo > 0 ? '#e8d9c0' : '#ff5a3c');
  } else if (!player.melee) {
    ptext('UNARMED', VIEW_W - 66, VIEW_H - 15, 7, 'rgba(232,217,192,0.45)');
  }
  ...
  if (scrapper.state !== 'dead' && Math.hypot(scrapper.x - player.x, scrapper.y - player.y) < 8)
  ...
  if (mission.state === 'active' || mission.state === 'complete') {
    ...
    ptext(mission.state === 'active'
      ? `Destroy the Scrapper - loot scrap ${Math.min(player.inv.scrap, 5)}/5`
      : 'Return to the survivor', 16, 8, 8);
  }

  // interaction prompt
  if (Prompt && !Dialog.active && !Trade.open) {
    const e = ptGet(Prompt.text, 8, '#ffd27a');
    const wpx = e.img.width;
    const tx = Prompt.sx - lastOx, ty = Prompt.sy - lastOy;
    uiRect(tx - wpx / 2 - 3, ty - 3, wpx + 6, 13, 'rgba(0,0,0,0.65)');
  ...
  if (InvUI.open) {
    ...
    for (let t = 0; t < 4; t++) {
      ...
      if (sel) uiRect(tx - 3, py0 + 4, INV_TABS[t].length * 6 + 6, 12, 'rgba(255,210,122,0.15)');
      ptext(INV_TABS[t], tx, py0 + 6, 8, sel ? '#ffd27a' : 'rgba(232,217,192,0.45)');
      tx += INV_TABS[t].length * 6 + 14;
    }
  ...
  // trade panel
  if (Trade.open) {
    const pw = 196, ph = 76, px0 = (VIEW_W - pw) / 2, py0 = (VIEW_H - ph) / 2 - 8;
    uiFrame(px0, py0, pw, ph);
    ...
    if (player.melee === 'knife') {
      ptext('[3] piercing knife', px0 + 26, py0 + 42, 8, 'rgba(232,217,192,0.4)');
      ptext('SOLD', px0 + pw - 8, py0 + 42, 8, 'rgba(232,217,192,0.4)', 'right');
    } else {
      ptext('[3] piercing knife', px0 + 26, py0 + 42, 8, can(15));
      ptext('15 scrap', px0 + pw - 8, py0 + 42, 8, can(15), 'right');
    }
    ptext(`your scrap: ${player.inv.scrap} · E close`, px0 + pw / 2, py0 + 61, 8, 'rgba(232,217,192,0.6)', 'center');
  }
