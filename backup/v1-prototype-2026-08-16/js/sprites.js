...
(function buildSprites() {
  ...
  function tile(base, specks, rows) {
    const c = makeCanvas(TILE_W, TILE_H), g = c.getContext('2d');
    for (let r = 0; r < TILE_H; r++) {
      const hw = diamondRow(r);
      px(g, 16 - hw, r, hw * 2, 1, base);
    }
    if (rows) {                       // plank-style darker rows
      for (let r = 2; r < TILE_H; r += 4) {
        const hw = diamondRow(r);
        px(g, 16 - hw, r, hw * 2, 1, rows);
      }
    }
    for (let i = 0; i < 44; i++) {
      const r = (rng() * TILE_H) | 0;
      const hw = diamondRow(r);
      const x = (16 - hw + rng() * hw * 2) | 0;
      const w = Math.min(1 + ((rng() * 2) | 0), 16 + hw - x);
      px(g, x, r, w, 1, specks[(rng() * specks.length) | 0]);
    }
    return c;
  }
  // palettes kept close together so type borders don't read as a grid
  Sprites.asphalt = []; Sprites.dirt = []; Sprites.rubble = []; Sprites.planks = [];
  for (let i = 0; i < 6; i++) {
    Sprites.asphalt.push(tile('#2c2a29', ['#262423', '#333030', '#29272a', '#3a3230', '#242220']));
    Sprites.dirt.push(tile('#3c322a', ['#332a22', '#463a2e', '#2e2620', '#41362a', '#37302c']));
    Sprites.rubble.push(tile('#34302c', ['#3e3833', '#2c2825', '#48403a', '#5a4632', '#302c29']));
  }
  for (let i = 0; i < 3; i++) {
    Sprites.planks.push(tile('#4a3a26', ['#55432c', '#3e301e', '#5c4a30', '#463726'], '#3e301e'));
  }
  ...
  (function () {
    ...
    // pistol icon for the HUD — compact Glock profile:
    // light grey serrated slide over a darker polymer frame and grip
    const p = makeCanvas(20, 13), pg = p.getContext('2d');
    px(pg, 2, 1, 1, 1, '#6a6d76');        // rear sight
    px(pg, 16, 1, 1, 1, '#6a6d76');       // front sight
    px(pg, 1, 2, 16, 4, '#8a8d96');       // slide
    px(pg, 1, 2, 16, 1, '#aaadb6');       // slide top highlight
    for (let i = 0; i < 3; i++) px(pg, 2 + i * 2, 3, 1, 2, '#6f727c');  // serrations
    px(pg, 11, 3, 4, 1, '#7a7d86');       // ejection port
    px(pg, 1, 6, 16, 1, '#6f727c');       // slide lower edge
    px(pg, 3, 7, 11, 2, '#3c3e46');       // frame
    px(pg, 7, 9, 1, 2, '#33353d');        // trigger guard loop
    px(pg, 7, 11, 3, 1, '#33353d');
    px(pg, 10, 9, 1, 1, '#33353d');
    px(pg, 8, 9, 1, 1, '#23252b');        // trigger
    px(pg, 12, 9, 4, 2, '#34363e');       // angled grip
    px(pg, 13, 11, 4, 2, '#31333b');
    px(pg, 13, 10, 1, 3, '#3e414b');      // grip texture
    px(pg, 14, 8, 2, 1, '#34363e');       // beavertail
